...
function renderTxTable(){
  ...
  b.innerHTML = page.length? page.map(d=>`<tr>
    ...
    <td data-label="N° Factura" class="mono small">${esc(d['NO COMPROBANTE'])}</td>
...
function renderVentasTable(){
  ...
  document.getElementById('vt-body').innerHTML = page.length? page.map(v=>`<tr>
    ...
    <td data-label="N° Doc" class="mono small">${esc(v['NO DOCUMENTO'])}</td>
...
function renderRetTable(){
  ...
  document.getElementById('ret-body').innerHTML = page.length? page.map(r=>`<tr>
    ...
    <td data-label="Sustento" class="mono small">${esc(r['DOCUMENTO SUSTENTO'])}</td>
...
function filterDiario(){
  ...
  diarFiltered=allEntries().filter(e=>{
    ...
    if(cta&&!e.lines.some(l=>l.nom===cta)) return false;
  ...
  return `<div class="asiento-card">
    <div class="asiento-header">${chk}<div class="asiento-num">${esc(e.id)}</div>
      <div class="asiento-meta"><div class="asiento-proveedor">${esc(e.concepto)}</div>
        <div class="asiento-detail"><span>📅 ${fmtDate(e.fecha)}</span><span>📌 ${esc(e.periodo)}</span><span>🔖 ${esc(e.ref)}</span><span>${tipo}</span></div></div>
...
function renderManuales(){
  ...
  const rows=MANUAL_ASIENTOS.filter(a=>!q||`${a.ref} ${a.concepto} ${a.periodo} ${a.lines.map(l=>l.cta+' '+l.nom).join(' ')}`.toLowerCase().includes(q));
  c.innerHTML = rows.length? rows.slice().reverse().map((a,i)=>{
    ...
    return `<div class="asiento-card"><div class="asiento-header">
      <div class="asiento-num">MAN-${String(rows.length-i).padStart(3,'0')}</div>
      <div class="asiento-meta"><div class="asiento-proveedor">${esc(a.concepto)}</div>
        <div class="asiento-detail"><span>📅 ${fmtDate(a.fecha)}</span><span>📌 ${esc(a.periodo)}</span><span>🔖 ${esc(a.ref)}</span>
        <span style="color:${Math.abs(d-h)<0.01?'var(--green)':'var(--red)'}">${Math.abs(d-h)<0.01?'✓ Cuadra':'⚠ $'+fmt(Math.abs(d-h))}</span></div></div>
      <span><button class="btn btn-ghost btn-sm" onclick="editManualAsiento(${a.id})">Editar</button>
      <button class="btn btn-danger btn-sm" onclick="deleteManualAsiento(${a.id})">Eliminar</button></span></div>
  ...
  }).join('') : '<div class="empty">Aún no hay asientos manuales.</div>';
}
...
function renderEstadoResultados(){
  ...
  document.getElementById('er-container').innerHTML=`<div class="fin-doc">${alerta}${finHead('Estado de Resultados Integral',per)}
    ...
    <div class="fin-row"><span>(-) 15% Participación Trabajadores (estimada)</span><span style="color:var(--red)">($${fmt(er.partTrab)})</span></div>
    <div class="fin-row"><span>(-) Impuesto a la Renta estimado (25%)</span><span style="color:var(--red)">($${fmt(er.irEstimado)})</span></div>
    ...
    <p class="text-muted small" style="margin-top:8px">La participación de trabajadores (15%) y el impuesto a la renta (25% tarifa general para sociedades) son estimaciones automáticas sobre la utilidad contable; la base imponible real requiere conciliación tributaria (gastos no deducibles, amortización de pérdidas, deducciones adicionales).</p>
