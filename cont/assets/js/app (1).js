...
function initFilters(){
  ...
  document.querySelectorAll('select[id$="-periodo"],select[id$="Periodo"]').forEach(sel=>{
  ...
  document.querySelectorAll('select[id*="cta-"],select[id*="cuenta"],select[id$="Cuenta"]').forEach(sel=>{
    ...
    if(!sel.options.length){
      ...
      const isDiarCuenta=sel.id==='diar-cuenta';
...
function getPeriodos(){
  const s=new Set();
  [getCompras(),getVentas(),getRetenciones()].forEach(arr=>{
    (arr||[]).forEach(r=>{if(r.periodo) s.add(r.periodo);});
  });
  (getAsientos()||[]).forEach(a=>{if(a.periodo) s.add(a.periodo);});
  const sorted=[...s].sort().reverse();
  return sorted.length?sorted:['2026-01','2026-02','2026-03','2026-04','2026-05','2026-06','2026-07','2026-08'];
}
