...
const CTA_N={
  ...
  iessPagar:{cod:'2.1.3.02',nom:'IESS por Pagar'},
...
function refreshNominaPeriodos(){
  ...
  let h='';
  for(let y=y0+1;y>=y0-8;y--){ h+=`<optgroup label="${y}">`;
...
function resetEmpleadoForm(){
  ...
  ['nemp-nombre','nemp-cedula','nemp-cargo','nemp-sueldo','nemp-fecha','nemp-quiro','nemp-prest'].forEach(i=>document.getElementById(i).value='');
...
function saveEmpleado(){
  ...
  const e={nombre,cedula:g('nemp-cedula'),cargo:g('nemp-cargo'),sueldo,fecha:g('nemp-fecha'),
    activo:document.getElementById('nemp-activo').value==='1',
...
function editEmpleado(id){
  ...
  s('nemp-fecha',e.fecha); s('nemp-activo',e.activo?'1':'0'); s('nemp-fr',e.frModo||'acumulado');
...
function mesesAntiguedad(f,per){
  if(!f||!per) return 0;
  const i=new Date(f+'T00:00:00'), [y,m]=per.split('-').map(Number), fin=new Date(y,m,0);
  if(isNaN(i)||fin<i) return 0;
  let ms=(fin.getFullYear()-i.getFullYear())*12+(fin.getMonth()-i.getMonth());
  if(fin.getDate()<i.getDate()) ms--;
...
function renderEmpleados(){
  ...
  b.innerHTML = NOMINA_EMPLEADOS.length? NOMINA_EMPLEADOS.map(e=>{
    const m=mesesAntiguedad(e.fecha,per), fr=m>=12;
  ...
  }).join('') : '<tr><td colspan="9" class="empty">Sin empleados registrados.</td></tr>';
}
function calcularNomina(){
  const per=document.getElementById('nom-periodo').value;
  const incluirFR=document.getElementById('nom-incluir-fr').checked;
  const act=NOMINA_EMPLEADOS.filter(e=>e.activo);
  if(!act.length) return showToast('No hay empleados activos','err');
  const c=NOMINA_CONFIG;
  const detalle=act.map(e=>{
    const sueldo=round2(e.sueldo), personal=round2(sueldo*c.personal/100), patronal=round2(sueldo*c.patronal/100);
    const d13=round2(sueldo/12), d14=round2(c.sbu/12), vac=round2(sueldo/24);
    const meses=mesesAntiguedad(e.fecha,per), aplicaFR=incluirFR&&meses>=12;
    ...
    const deduc=round2(personal+quiro+prestamo+ext+anticipo);
    const netoBase=round2(sueldo-deduc);
    return {...e,sueldo,personal,patronal,d13,d14,vac,meses,aplicaFR,fr,frMensual,frAcum,
      d13M,d13A,d14M,d14A,quiro,prestamo,ext,anticipo,deduc,netoBase,
...
function renderNominaPreview(){
  ...
  c.innerHTML=`<div class="table-wrap"><table class="rtable"><thead><tr>
    ...
    <div class="asiento-total"><span>Total DEBE (gasto): <strong class="debe">$${fmt(round2(tot.sueldo+tot.patronal+tot.d13+tot.d14+tot.vac+tot.fr))}</strong></span></div>
...
function construirLineasNomina(t){
  return [
    ...
    {...CTA_N.sueldosPagar,cta:CTA_N.sueldosPagar.cod,debe:0,haber:t.netoBase},
    {...CTA_N.iessPagar,cta:CTA_N.iessPagar.cod,debe:0,haber:round2(t.personal+t.patronal+t.quiro+t.ext)},
    {...CTA_N.anticipos,cta:CTA_N.anticipos.cod,debe:0,haber:t.anticipo},
    {...CTA_N.prestamos,cta:CTA_N.prestamos.cod,debe:0,haber:t.prestamo},
    {...CTA_N.d13Pagar,cta:CTA_N.d13Pagar.cod,debe:0,haber:t.d13},
    {...CTA_N.d14Pagar,cta:CTA_N.d14Pagar.cod,debe:0,haber:t.d14},
    {...CTA_N.vacPagar,cta:CTA_N.vacPagar.cod,debe:0,haber:t.vac},
    {...CTA_N.frPagar,cta:CTA_N.frPagar.cod,debe:0,haber:t.fr}
...
function abrirRevisionNomina(){
  ...
  document.getElementById('revision-nomina-info').innerHTML=`<strong>Nómina ${esc(periodLabel(nomCalc.periodo))}</strong> · ${nomCalc.detalle.length} empleado(s)<br>
    <span class="text-muted">Revisa y ajusta las líneas antes de aprobar.</span>`;
...
function lineasPorPagar(t){
  return [
    {cta:CTA_N.sueldosPagar.cod,nom:CTA_N.sueldosPagar.nom,valor:t.netoBase!=null?t.netoBase:round2(t.neto-(t.frMensual||0)),porBanco:true},
    {cta:CTA_N.iessPagar.cod,nom:CTA_N.iessPagar.nom,valor:round2(t.personal+t.patronal+(t.quiro||0)+(t.ext||0)),porBanco:true},
    {cta:CTA_N.frPagar.cod,nom:CTA_N.frPagar.nom,valor:t.fr||0,porBanco:true},
    {cta:CTA_N.d13Pagar.cod,nom:CTA_N.d13Pagar.nom,valor:t.d13,porBanco:false},
    {cta:CTA_N.d14Pagar.cod,nom:CTA_N.d14Pagar.nom,valor:t.d14,porBanco:false},
    {cta:CTA_N.vacPagar.cod,nom:CTA_N.vacPagar.nom,valor:t.vac,porBanco:false}
  ].filter(l=>l.valor>0);
}
...
function renderNominaHistorial(){
  ...
  c.innerHTML = pers.length? pers.map(per=>{
    ...
    return `<div class="asiento-card"><div class="asiento-header">
      <div class="asiento-meta"><div class="asiento-proveedor">${esc(r.ref)} · ${esc(periodLabel(per))}
      ...
      ${pagos.length?`<div class="table-wrap" style="max-height:none;border:none"><table class="rtable"><thead><tr><th>Fecha</th><th>Banco</th><th>Cuentas</th><th class="num">Monto</th><th></th></tr></thead>
        <tbody>${pagos.map(p=>`<tr><td data-label="Fecha">${fmtDate(p.fecha)}</td><td data-label="Banco">${esc(p.banco?.nom||'-')}</td>
...
function exportRolesHistorialPDF(per){ const r=NOMINA_RUNS[per]; rolesPDF(per,r&&r.detalle); }
