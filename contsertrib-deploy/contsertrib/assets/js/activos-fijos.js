...
function bajaActivo(id, motivo, precioVenta){
  ...
  const asiento = { fecha: a.fechaBaja, periodo: a.fechaBaja.slice(0,7), concepto: 'Baja de activo fijo: '+a.nombre, ref: a.codigo||a.id, lines, nomina: false };
...
function generarDepreciacionMensual(periodo){
  ...
  const asiento = { fecha: periodo+'-28', periodo, concepto: 'Depreciación mensual de activos fijos', ref: 'DEP-'+periodo, lines: Object.values(consolidado), nomina: false };
  MANUAL_ASIENTOS.push(asiento);
  persistManuales();
  saveAF();
  invalidateEntries();
...
function buildActivosView(container){
  ...
  if(filtered.length === 0){
    ...
    filtered.forEach(a => {
      ...
      const btnBaja = a.estado==='activo' ? '<button class="btn-sm btn-warn" onclick="bajaActivoFijo(\''+a.id+'\')" title="Dar de baja"><i class="fa-solid fa-circle-minus"></i></button>' : '';
...
function openModalActivoFijo(id){
  ...
  h += '</div>';
  h += '<div class="modal-actions"><button class="btn btn-primary" onclick="saveActivoFijo(\''+(id||'')+'\')"><i class="fa-solid fa-floppy-disk"></i> '+(a?'Actualizar':'Registrar')+'</button><button class="btn" onclick="closeModal(\'modal-generic\')">Cancelar</button></div>';
  h += '</div>';
...
function bajaActivoFijo(id){
  ...
  h += '<div class="modal-actions"><button class="btn btn-warning" onclick="confirmarBajaActivo(\''+id+'\')"><i class="fa-solid fa-circle-minus"></i> Confirmar Baja</button><button class="btn" onclick="closeModal(\'modal-generic\')">Cancelar</button></div></div>';
